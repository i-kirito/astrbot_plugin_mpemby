# MoviePilot & Emby 插件

> 本项目是基于 AstrBot 开发的插件，提供 MoviePilot 订阅管理和 Emby 媒体库查询/统计功能。

## ✨ 功能特性

- **MoviePilot**:
  - 影片搜索与订阅 (支持交互式选片/选季)
  - 查看当前订阅列表
  - 查看下载进度
- **Emby**:
  - 查看最新入库 (电影/剧集)
  - 媒体库搜索
  - 媒体库统计
  - **每日入库日报推送** (定时统计今日新增)

## ⚙️ 配置说明

请在 AstrBot 管理后台的插件配置中填写：

### MoviePilot 配置
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `mp_url` | MP 地址 (e.g. `http://192.168.1.2:3000`) | `http://localhost:3000` |
| `mp_token` | MP Token | - |
| `mp_username` | 用户名 | `admin` |
| `mp_password` | 密码 | - |

### Emby 配置
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `emby_url` | Emby 地址 (e.g. `http://192.168.1.2:8096`) | `http://localhost:8096` |
| `emby_api_key` | API Key (在 Emby 后台生成) | - |
| `emby_user_id` | 用户 ID (用于获取特定用户的视图) | - |

### 日报推送配置
| 配置项 | 说明 |
|--------|------|
| `enable_daily_report` | 是否开启每日推送 (默认关闭) |
| `report_time` | 推送时间 (格式 `HH:MM`, 默认 `20:00`) |
| `report_target_id` | **推送目标 ID** (QQ号或群号) |

## 💻 指令列表

### MoviePilot
| 指令 | 说明 |
|------|------|
| `/mp订阅 [片名]` | 搜索并订阅影片 |
| `/mp当前订阅` | 查看当前订阅列表（仅显示订阅中） |
| `/mp下载` | 查看当前下载进度 |

### Emby
| 指令 | 说明 |
|------|------|
| `/emby [类型]` | 查看最新入库 (可选: 电影/电视剧/全部) |
| `/emby搜索 [关键词]` | 在媒体库中搜索 |
| `/emby统计` | 查看媒体库统计信息 |

### 推送管理 (管理员)
| 指令 | 说明 |
|------|------|
| `/emby推送配置` | 查看/修改推送设置 (支持 on/off/time/target) |
| `/emby推送` | 手动触发一次今日日报推送 |

### 其他
| 指令 | 说明 |
|------|------|
| `/订阅帮助` | 显示帮助信息 |

## 📝 版本历史

- **v1.2.9**:
  - 修复：订阅时只响应发起订阅的用户，忽略其他用户消息
- **v1.2.8**:
  - 新增：订阅成功后发送图片卡片通知（可在后台关闭）
  - 新增：回复订阅请求时引用用户消息
  - 新增：`enable_subscribe_card` 配置项控制卡片开关
- **v1.2.7**:
  - 优化：电视剧订阅直接订阅所有季，无需手动选择
- **v1.2.6**:
  - 优化：推送改为文本渲染图片形式发送
  - 优化：同一剧集不同集数合并显示（如 S1 E1-E3, E5-E6）
- **v1.2.5**:
  - 新增：`/mp当前订阅` 指令，查看当前订阅列表（仅显示订阅中的，过滤已完成）
- **v1.2.4**:
  - 优化：代码结构调整
- **v1.2.3**:
  - 修复：推送功能因 API 调用问题导致的报错 (PlatformManager)。
  - 优化：增强了对不同 AstrBot 版本的适配器兼容性。
- **v1.2.2**:
  - 增强日报功能：推送内容增加具体的影片名称列表。
  - 优化手动推送：`/emby推送` 指令现在会有明确的反馈（即使无更新）。
- **v1.2.1**:
  - 指令全面汉化。
  - 新增每日入库日报推送功能。
  - 优化默认配置提示。
- **v1.2.0**:
  - 新增 Emby 入库查询、搜索、统计功能。
