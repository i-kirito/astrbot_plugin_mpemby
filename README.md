...
### v1.3.4
- [Security] **安全加固**：API 请求日志脱敏处理，不再暴露 Token 和密码
- [Security] **空值防护**：所有 API 方法增加 headers 空值检查，避免未认证请求
- [Security] **URL 编码**：用户输入的搜索关键词现在经过 URL 编码，防止注入
- [Fix] **临时文件清理**：使用 try/finally 确保卡片图片临时文件被正确清理
- [Optimize] **配置优化**：全面优化 `_conf_schema.json` 描述和提示

### v1.3.2
- [Feat] **订阅成功卡片**：全新现代风格设计，渐变背景、圆角卡片、图标式信息展示

### v1.3.0 ~ v1.3.1
- [Fix] 订阅时只响应发起订阅的用户，忽略其他用户消息
- [Feat] 订阅成功后发送图片卡片通知（可在后台关闭）
- [Feat] 回复订阅请求时引用用户消息

### v1.2.5 ~ v1.2.9
- [Feat] 新增 `/mp当前订阅` 指令，查看当前订阅列表
- [Optimize] 电视剧订阅直接订阅所有季，无需手动选择
- [Optimize] 推送改为文本渲染图片形式发送
- [Optimize] 同一剧集不同集数合并显示

### v1.2.0 ~ v1.2.4
- [Feat] 新增 Emby 入库查询、搜索、统计功能
- [Feat] 新增每日入库日报推送功能
- [Feat] 指令全面汉化
- [Fix] 推送功能 API 调用兼容性修复
