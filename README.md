# MoviePilot & Emby 插件

> 本项目是基于 AstrBot 开发的插件，提供 MoviePilot 订阅管理和 Emby 媒体库查询/统计功能。

## ✨ 功能特性

- **MoviePilot**:
  - 影片搜索与订阅 (支持交互式选片/选季)
  - 查看当前订阅列表
  - 查看下载进度
- **Emby**:
  - 查看最新入库 (电影/剧集)
  - 媒体库搜索
  - 媒体库统计
  - **每日入库日报推送** (定时统计今日新增)

## ⚙️ 配置说明

请在 AstrBot 管理后台的插件配置中填写：

### MoviePilot 配置
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `mp_url` | MP 地址 (e.g. `http://192.168.1.2:3000`) | `http://localhost:3000` |
| `mp_token` | MP Token | - |
| `mp_username` | 用户名 | `admin` |
| `mp_password` | 密码 | - |

### Emby 配置
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `emby_url` | Emby 地址 (e.g. `http://192.168.1.2:8096`) | `http://localhost:8096` |
| `emby_api_key` | API Key (在 Emby 后台生成) | - |
| `emby_user_id` | 用户 ID (用于获取特定用户的视图) | - |

### 日报推送配置
| 配置项 | 说明 |
|--------|------|
| `enable_daily_report` | 是否开启每日推送 (默认关闭) |
| `report_time` | 推送时间 (格式 `HH:MM`, 默认 `20:00`) |
| `report_target_id` | **推送目标 ID** (QQ号或群号) |

## 💻 指令列表

### MoviePilot
| 指令 | 说明 |
|------|------|
| `/mp订阅 [片名]` | 搜索并订阅影片 |
| `/mp当前订阅` | 查看当前订阅列表（仅显示订阅中） |
| `/mp下载` | 查看当前下载进度 |

### Emby
| 指令 | 说明 |
|------|------|
| `/emby [类型]` | 查看最新入库 (可选: 电影/电视剧/全部) |
| `/emby搜索 [关键词]` | 在媒体库中搜索 |
| `/emby统计` | 查看媒体库统计信息 |

### 推送管理 (管理员)
| 指令 | 说明 |
|------|------|
| `/emby推送配置` | 查看/修改推送设置 (支持 on/off/time/target) |
| `/emby推送` | 手动触发一次今日日报推送 |

### 其他
| 指令 | 说明 |
|------|------|
| `/订阅帮助` | 显示帮助信息 |

## 📝 版本历史

### v1.3.4 (2026-1-29)
- [Security] API 请求日志脱敏处理，不再暴露 Token 和密码
- [Security] 所有 API 方法增加 headers 空值检查
- [Security] 用户输入的搜索关键词经过 URL 编码，防止注入
- [Fix] 使用 try/finally 确保卡片图片临时文件被正确清理
- [Optimize] 全面优化 `_conf_schema.json` 描述和提示

### v1.3.2 (2026-1-28)
- [Feat] 订阅成功卡片全新现代风格设计

### v1.3.0 ~ v1.3.1 (2026-1-27)
- [Fix] 订阅时只响应发起订阅的用户
- [Feat] 订阅成功后发送图片卡片通知
- [Feat] 回复订阅请求时引用用户消息

### v1.2.5 ~ v1.2.9 (2026-1-26)
- [Feat] 新增 `/mp当前订阅` 指令
- [Optimize] 电视剧订阅直接订阅所有季
- [Optimize] 推送改为文本渲染图片形式发送
- [Optimize] 同一剧集不同集数合并显示

### v1.2.0 ~ v1.2.4 (2026-1-25)
- [Feat] Emby 入库查询、搜索、统计功能
- [Feat] 每日入库日报推送功能
- [Feat] 指令全面汉化
- [Fix] 推送功能 API 调用兼容性修复
